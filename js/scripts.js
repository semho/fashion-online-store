...
if (shopList) {
  ...
  shopList.addEventListener('click', (evt) => {
    ...
    const currentProdId = evt.target.querySelector('.product__id').textContent;
    const currentProdPrice = evt.target.querySelector('.product__price').textContent;
    ...
    if (prod.some(pathItem => pathItem.classList && pathItem.classList.contains('shop__item'))) {
      ...
      buttonOrder.addEventListener('click', (evt) => {
        ...
        //отменяем оформление заказа из этого скрипта
        const serverValidate = false;

        //если все поля заполнены
        if (inputs.every(inp => !!inp.value)) {
            evt.preventDefault();
            //передаем форму и id товара в скипт main.js
            sendToServer(form, currentProdId, currentProdPrice);
        }


        if (serverValidate && inputs.every(inp => !!inp.value)) {
...
//если мы хотим изменить товар в админке
const addListChange = document.querySelector('.add-list-change');
if (addList) {
    ...
    if (addListChange) {
        document.querySelector('.add-list__item--active').addEventListener('click', evt => {
            addList.removeChild(evt.target);
            addInput.value = '';
            checkList(addList, addButton);
            addListChange.classList.remove('add-list-change');
        });
    }
...
if (productsList) {
  ...
  productsList.addEventListener('click', evt => {
    ...
    if (target.classList && target.classList.contains('product-item__delete')) {
        ...
        //функция делает неактивным товар(скрипт в main.js)
        inactiveProduct();
    }
...
if (document.querySelector('.shop-page')) {
    ...
    const minPrice = parseInt($('.min-price').text().replace(/\D/g, ""));
    const maxPrice = parseInt($('.max-price').text().replace(/\D/g, ""));
